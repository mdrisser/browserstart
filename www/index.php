...
<html lang="en">
    ...
    <body>
        ...
        <!-- WEATHER -->
            <div class="columns">
                <div class="column col-3 wx">
                    <div>Bullhead City WX CONDX</div>
                    <div class="h2 temp"><?= round($bhc_wx->main->temp, 0); ?>&deg;F</div>
                    <div class="wind-speed">Wind: <?= round($bhc_wx->wind->speed, 0); ?>MPH (<?= $bhc_wx->wind->deg; ?>)</div>
                    <div class="wind-gust">Gust: <?= round($bhc_wx->wind->gust, 0); ?>MPH</div>
                    <div class="humidity">Humidity: <?= round($bhc_wx->main->humidity, 0); ?>%</div>
                </div>
                 <div class="column col-3 wx">
                    <div>Kingman WX CONDX</div>
                    <div class="h2 temp"><?= round($kmn_wx->main->temp, 0); ?>&deg;F</div>
                    <div class="wind-speed">Wind: <?= round($kmn_wx->wind->speed, 0); ?>MPH (<?= $kmn_wx->wind->deg; ?>)</div>
                    <div class="wind-gust">Gust: <?= round($kmn_wx->wind->gust, 0); ?>MPH</div>
                    <div class="humidity">Humidity: <?= round($kmn_wx->main->humidity, 0); ?>%</div>
                </div>
            </div>
        <!-- RSS FEEDS -->
            <?php
